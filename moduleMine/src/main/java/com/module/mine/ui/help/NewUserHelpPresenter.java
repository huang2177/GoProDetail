...
public class NewUserHelpPresenter extends BasePresenter<NewUserHelpView> {
    ...
    public void getHelpDetalis(String helpId) {
        observer(new HttpObserver<HelpDetailBean>(mContext, mService.getProblemDetalis(helpId), new HttpCallBackImpl<HelpDetailBean>() {
            @Override
            public void onCompleted(HelpDetailBean detalisBean) {
                if (detalisBean == null) {
                    return;
                }
                helpView.showHelpDetail(detalisBean.getData());
            }
        }));
    }
